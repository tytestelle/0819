...
public class DatePicker extends Picker {
    ...
    public List<CharSequence> extractSeparators() {
        ...
        for (int i = 0; i < bestYearMonthDayPattern.length(); i++) {
            char cCharAt = bestYearMonthDayPattern.charAt(i);
            if (cCharAt != ' ') {
                if (cCharAt != '\'') {
    ...
    public void setDatePickerFormat(String str) {
        ...
        if (listExtractSeparators.size() != str.length() + 1) {
            throw new IllegalStateException("Separators size: " + listExtractSeparators.size() + " must equal the size of datePickerFormat: " + str.length() + " + 1");
        }
    ...
    public void updateSpinnersImpl(boolean z) {
        ...
        for (int length = DATE_FIELDS.length - 1; length >= 0; length--) {
            ...
            if (i >= 0) {
                ...
                boolean zUpdateMin = (z2 ? updateMin(columnAt, this.mMinDate.get(i2)) : updateMin(columnAt, this.mCurrentDate.getActualMinimum(i2))) | (z3 ? updateMax(columnAt, this.mMaxDate.get(i2)) : updateMax(columnAt, this.mCurrentDate.getActualMaximum(i2)));
    ...
    public DatePicker(Context context, AttributeSet attributeSet, int i) {
        ...
        try {
            ...
            if (TextUtils.isEmpty(string) || !parseDate(string, this.mTempDate)) {
                this.mTempDate.set(Constants.UPNP_MULTICAST_PORT, 0, 1);
            }
