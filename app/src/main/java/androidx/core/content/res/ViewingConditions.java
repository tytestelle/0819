...
final class ViewingConditions {
    ...
    public static ViewingConditions make(@NonNull float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        float f15 = 1.0f - f14;
        float fCbrt = (0.1f * f15 * f15 * ((float) Math.cbrt(((double) f) * 5.0d))) + (f14 * f);
        ...
        return new ViewingConditions(fYFromLStar, ((fArr8[2] * 0.05f) + (fArr8[0] * 2.0f) + fArr8[1]) * fPow, fPow, fPow, fLerp, f12, fArr6, fCbrt, (float) Math.pow(fCbrt, 0.25d), fSqrt);
    }
